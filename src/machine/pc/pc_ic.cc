...
void IC::entry()
{
    // For multitasking configurations, there are two cases for us to get here: same-level (we are already inside the kernel and an interrupt occurs)
    // and cross-level interrupts. Single-tasks are always same-level.
    // When an interrupt occurs at user level, before pushing FLAGS, CS, and IP into the stack, the CPU first pushes the user-level SS and SP.
    // While returning, the CPU checks for the PL bits of FLAGS to identify the stack layout and then takes the proper procedures.
    // So, the stack contents at this point are:
    //   same-level: FLAGS, CS, IP;
    //   cross-level: previous-level SS, previous-level SP, FLAGS, CS, IP.

    static int id;  // id must be static because we don't have a stack frame
